...
Универсальное позиционирование для Enterprise, ТЭК, Финтех, Ритейл.
Курированная витрина: личные флагманские проекты + по 2 лучших партнерских проекта.
"""
import sys
import os
...
def create_portfolio():
    ...
    add_callout(
        ...
        "Специалист редкого гибридного профиля на стыке архитектуры ИИ-систем, системной инженерии и продуктового управления:\n"
    ...
    # 4. ФУНКЦИИ ВЫВОДА КАРТОЧЕК ПРОЕКТОВ
    ...
    # ----------------------------------------------------
    # БЛОК 1: МУЛЬТИАГЕНТНЫЕ СИСТЕМЫ И КОРПОРАТИВНЫЙ GENAI
    # ----------------------------------------------------
    add_section_header("2. Мультиагентные системы и корпоративный GenAI (Enterprise AI Automation)")
    ...
    add_project_card(
        ...
        "Агент 1 собирает отраслевые сигналы без платных API и формирует ранние предупреждения (Early Warnings). "
        "Агент 2 синтезирует инициативы и валидирует их в цикле рефлексии (Reflexion) по критериальной матрице с масштабированием до 1000 критериев (Map-Reduce). "
        "Агент 3 адаптирует инициативы под отраслевой сленг подразделений и генерирует презентации по брендбуку.",
    ...
    add_project_card(
        "Multi-Agent Judge: Двухагентный контур самокритики и арбитража решений (Evaluator-Optimizer)",
        "AI Solutions Architect (в партнерстве со специалистами из технологических компаний)",
        "Python, Multi-Agent Architecture, Anthropic Building Effective Agents, LLM-as-a-Judge, Ragas.",
        "Архитектурный контур из 4-х агентов: Генератор решения -> Тестировщик дефектов -> Оптимизатор -> Независимая Модель-Судья (Judge). "
        "Судья выставляет объективный скоринг (0-10) и при непрохождении порога возвращает задачу на доработку с замечаниями. "
        "Полностью детерминированный цикл рефлексии (Reflexion), гарантирующий исключение галлюцинаций до публикации артефактов.",
        "Действующий модуль agent_evaluator_optimizer.py, 100% покрытие автотестами, детерминированная критериальная матрица."
    )

    add_project_card(
        "OilGas FastMCP Server: Высокопроизводительный сервер контекста по нормативной базе ТЭК",
        ...
        "Разработка сервера контекста новейшего стандарта Anthropic MCP для интеграции LLM-агентов с нормативной базой (ГОСТ Р 53711, СТО, ФНП № 534). "
        "Позволяет любому внешнему агенту через stdio и стандартизированный JSON-RPC безопасно извлекать точные нормативные требования с миллисекундным откликом.",
        "Действующий MCP-сервер с инструментами семантического поиска и верификации стандартов. Тестовый скрипт test_oilgas_mcp.py."
    )

    # ----------------------------------------------------
    # БЛОК 2: ИНДУСТРИАЛЬНЫЙ ML И ТЕЛЕМЕТРИЯ В ТЭК И ГМК
    # ----------------------------------------------------
    add_section_header("3. Industrial AI, промышленная телеметрия и MLOps (ТЭК и Тяжелая индустрия)")

    add_project_card(
        "Цифровая трансформация ТОиР и контур раннего предупреждения (Восточная горнорудная компания)",
        "Руководитель проектов цифровой трансформации (производственное внедрение)",
        ...
        "Действующий производственный контур, сокращение времени регистрации инцидентов ТОиР, система предиктивного мониторинга отказов техники."
    )
    ...
    add_project_card(
        ...
        "Автоматическое выявление аномальных технологических режимов и предупреждение рисков прихвата бурового инструмента.",
    ...
    add_project_card(
        ...
        "Обработка потока до 5 000 замеров/сек, предотвращение аварийного подземного ремонта скважин (ПРС). Модуль quick_score_demo.py."
    )

    # ----------------------------------------------------
    # БЛОК 3: ФИНТЕХ, LEGALTECH И АВТОМАТИЗАЦИЯ РИСКОВ
    # ----------------------------------------------------
    add_section_header("4. FinTech, LegalTech и системы принятия решений (Decision Engines)")

    add_project_card(
        "Экспресс-аудит коммерческих договоров B2B и скоринг юридических рисков",
        "Автор решения / Developer",
        "Python 3.9+, Pydantic v2, Decision Engine, стоп-факторы Red Flags, автогенерация протоколов.",
        "Автоматический аудит коммерческих договоров поставки и подряда: детекция кабальных условий (отсрочка платежа >90 дней, односторонняя пеня >0.1%/день, "
        "отсутствие форс-мажора), расчет скоринга надежности контракта (0-100) и автоматическая генерация официального протокола разногласий.",
        "Работающий скрипт run.py, структурированный отчет audit.json, готовый юридический протокол protocol_of_disagreements.md."
    )

    add_project_card(
        "Telecom Churn Prediction: Прогнозирование оттока клиентов телеком-оператора",
        "Data Analyst / ML-инженер",
        "Python, Pandas, Scikit-learn, XGBoost, Matplotlib, Jupyter Notebook (7 043 клиента, 21 признак).",
        "Разведочный анализ данных (EDA), выявление ключевых факторов оттока, балансировка выборки, сравнительное обучение 3-х моделей "
        "(Logistic Regression, Random Forest, XGBoost) и разработка прикладных рекомендаций для CRM по удержанию клиентской базы.",
        "Лучшая модель с метриками AUC-ROC = 0.8425, Accuracy = 0.8034. Топ-факторы риска: тип контракта month-to-month, tenure, MonthlyCharges."
    )

    add_project_card(
        "Bank Marketing Campaign Analytics: Аналитический дашборд в Yandex DataLens",
        "Бизнес-аналитик / Data Analyst",
        "Yandex DataLens, SQL, Excel, датасет UCI Bank Marketing (41 188 записей).",
        "Анализ конверсии маркетинговых кампаний банка по привлечению депозитов: расчет 3 KPI, построение 5 интерактивных визуализаций, "
        "сегментация клиентов и выявление ключевых паттернов (звонки длительностью 5+ минут обеспечивают трехкратный рост конверсии).",
        "Интерактивный BI-дашборд с кросс-фильтрацией, прикладные рекомендации по оптимизации сезонных маркетинговых бюджетов."
    )

    # ----------------------------------------------------
    # БЛОК 4: ПРОДУКТОВЫЕ СЕРВИСЫ, РИТЕЙЛ И E-COMMERCE
    # ----------------------------------------------------
    add_section_header("5. Продуктовые сервисы, Ритейл и E-commerce Аналитика")
    ...
    add_project_card(
        ...
        "Проектирование архитектуры ИИ-агента для прогнозирования потребительского спроса на 14 дней вперед для розничной сети: "
        "формализация бизнес-процессов в BPMN, проектирование User Flow веб-интерфейса, постановка ТЗ для ML-модели и ETL-контуров, "
        "реестр рисков качества данных и координация спринтов команды.",
        "Прототип с точностью ~75% на исторических данных, полный комплект проектных спецификаций, целевое снижение дефицита полок на 15%."
    )

    add_project_card(
        "Умный мониторинг рынка и цен конкурентов (E-commerce Retail Analytics)",
        "Автор решения / Developer",
        "Python 3.9+, Pydantic v2, потоковая валидация данных, HTML5/CSS3 дашборды.",
        "Автоматический мониторинг ценообразования и складских остатков: детекция недобросовестного демпинга (>20%), дефицита на складах конкурентов "
        "и всплесков рекламаций. Автоматическое формирование сводки управленческих рекомендаций для коммерческого директора.",
        "Интерактивный адаптивный дашборд competitor_dashboard.html, машиночитаемый журнал сигналов competitor_analysis.json."
    )
    ...
    add_project_card(
        ...
        "Разработка Telegram-бота для владельцев детейлинг-центров: автоматическая генерация постов и описаний услуг с сохранением Tone of Voice. "
        "Проведение CustDev (интервью с 50+ предпринимателями), расчет юнит-экономики (LTV/CAC, модель монетизации по подписке).",
        "Рабочий прототип бота, 78% контента принимается заказчиками без ручных правок, сокращение времени подготовки поста с 45 мин до 30 сек."
    )

    # 5. УПРАВЛЕНЧЕСКИЙ ТРЕК-РЕКОРД
    ...
    cases_data = [
        ("Управление масштабными офлайн-событиями (до 3 000 участников)", "Проджект-менеджер, организатор", "Организация мероприятий крупного масштаба с привлечением более 10 субподрядчиков (звук/свет, безопасность, кейтеринг). 100% соблюдение сметы, полные аншлаги, эффективный кризис-менеджмент."),
        ...
        ("Запуск и операционное руководство товарным бизнесом", "Инициатор, операционный директор", "CustDev-исследование ниши, аудит фабрик, брендинг, логистика и контроль маржинальности. Успешный запуск и выход на плановую окупаемость."),
        ("Организация спортивного турнира полного цикла", "Инициатор и проджект-менеджер", "Любительский турнир (12 команд, 200+ участников, зрители, судейская коллегия). Соблюдение сметы и сроков на 100%, получение плановой прибыли.")
    ]
    ...
    print(f"Документ портфолио успешно сформирован: {output_path}")
